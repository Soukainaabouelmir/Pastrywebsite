...
.navbar .navbar-nav .nav-link.activeeeee {
    color:#e91e63;
}
...
<body>
    ...
    <section>
        <h2 class="section-title">Buffet</h2>
...
<div class="container my-5">
    <h1 class="text-center mb-4" style="color: #e91e63;">Menu des Buffet</h1>
    @if ($hasProducts)
    ...
    @else
        <p class="text-center" style="color: #e91e63;">Il n'y a pas de gâteaux de ce type pour le moment.</p>
    @endif
</div>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  $(document).ready(function() {
    $('#typeButton').click(function() {
        ...
        $.ajax({
            url: '/getbuffet-cake-types', // L'URL de votre route pour récupérer les types
            type: 'GET',
...
function filterProductsByType(type) {
    $.ajax({
        url: '/filter/buffet-products-by-type', // L'URL de votre route pour filtrer par type
        type: 'GET',
        data: {
        ...
        success: function(data) {
            ...
            if (data.products.length === 0) {
                $('#productsContainer').append('<p>Aucun produit trouvé pour ce type.</p>');
            } else {
                $.each(data.products, function(index, product) {
                    $('#productsContainer').append(`
                        <div class="col-md-4">
                            <div class="card mb-3">
                                <img src="{{ asset($product->image) }}" class="card-img-top" alt="${product.nom}"> <!-- Corrigé ici -->
                                <div class="card-body">
                                    <h5 class="card-title">${product.nom}</h5>
                                    <p class="card-text">${product.description}</p>
                                    <p class="text-muted">Prix : ${product.prix} €</p>
...
<script>
    $(document).ready(function() {
        ...
        $('#prixModal .btn-secondary').click(function() {
            var selectedPrice = $('#priceRange').val(); // Récupérer la valeur du prix

            $.ajax({
                url: '/filter/buffet-products',
                type: 'GET',
                data: {
                    price: selectedPrice // Envoyer la valeur sélectionnée comme paramètre
                },
                success: function(data) {
                    $('#productsContainer').empty(); // Vider le conteneur des produits existants
                    if (data.products.length === 0) {
                        $('#productsContainer').append('<p>Aucun produit trouvé pour ce prix.</p>');
                    } else {
                        $.each(data.products, function(index, product) {
                            $('#productsContainer').append(`
                                <div class="col-md-4">
                    ...
                    $('#prixModal').modal('hide'); // Fermer le modal après l'affichage des résultats
                },
                error: function(xhr) {
                    console.error(xhr.responseText);
                }
    ...
    function updatePrice(value) {
        document.getElementById('priceValue').innerText = value; // Mettre à jour l'affichage du prix
    }
</script>
